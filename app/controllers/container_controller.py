...
def create_container():
    ...
    shipping_company = ShippingCompany.query.filter_by(id_user=user['id']).first()

    data['id_shipping_company'] = shipping_company.id
...
def get_container_by_tracking_code(tracking_code: int):
    ...
    container = Container.query.filter_by(tracking_code=tracking_code)
    ...
    if shipping_company['id'] != container['id_shipping_company']:
...
def update_container_by_tracking_code(tracking_code: int):
    ...
    try:
        ...
        updated_container = Container.query.get(tracking_code=tracking_code)

        return jsonify(updated_container), HTTPStatus.OK
    except TypeError:  # TODO: mudar o tipo do erro para um mais específico
